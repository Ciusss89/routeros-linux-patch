...
static inline irqreturn_t msm_spi_qup_irq(int irq, void *dev_id)
{
	...
	if (readl_relaxed(dd->base + SPI_ERROR_FLAGS) ||
		...
		return msm_spi_error_irq(irq, master);
	}
	...
	spin_lock(&dd->op_lock);
	dd->op |= op;
	tasklet_schedule(&dd->tasklet);
	spin_unlock(&dd->op_lock);
	return IRQ_HANDLED;
}

static void msm_spi_tasklet(unsigned long data) {
	int irq = 0;
	struct msm_spi *dd = (struct msm_spi *)data;
	u32 op;
	unsigned long flags;
	spin_lock_irqsave(&dd->op_lock, flags);
	op = dd->op;
	dd->op = 0;
	spin_unlock_irqrestore(&dd->op_lock, flags);

	if (op & SPI_OP_INPUT_SERVICE_FLAG) {
		msm_spi_input_irq(irq, dd);
	}

	if (op & SPI_OP_OUTPUT_SERVICE_FLAG) {
		msm_spi_output_irq(irq, dd);
	}
...
skip_dma_resources:
	...
	tasklet_init(&dd->tasklet, msm_spi_tasklet, (unsigned long)dd);
	spin_lock_init(&dd->op_lock);
