...
static void export_specific_button_gpio(int num) {
    static const char *gpio_names[GPIO_COUNT];
    gpio_names[num] = "button";
    gpio_to_chip(num)->names = gpio_names;
    gpio_request(num, "button");
    gpio_export(num, false);
}

static int export_button_gpio(void)
{
    if (mips_machtype == MACH_MT_RB941HL) {
	export_specific_button_gpio(16);
    }
    return 0;
}
device_initcall(export_button_gpio);
