...
extern atomic_t fp_ipv4_disable_count;
extern void (*fp_ipv4_set)(int);
static inline void fp_ipv4_disable(void) {
    if (atomic_inc_return(&fp_ipv4_disable_count) == 1) {
	fp_ipv4_set(0);
    }
}
static inline void fp_ipv4_enable(void) {
    if (atomic_dec_return(&fp_ipv4_disable_count) == 0) {
	fp_ipv4_set(1);
    }
}

extern atomic_t fp_ipv4_ct_disable_count;
extern void (*fp_ipv4_ct_set)(int);
static inline void fp_ipv4_ct_disable(void) {
    if (atomic_inc_return(&fp_ipv4_ct_disable_count) == 1) {
	fp_ipv4_ct_set(0);
    }
}
static inline void fp_ipv4_ct_enable(void) {
    if (atomic_dec_return(&fp_ipv4_ct_disable_count) == 0) {
	fp_ipv4_ct_set(1);
    }
}

extern atomic_t fp_bridge_disable_count;
extern void (*fp_bridge_set)(int);
static inline void fp_bridge_disable(void) {
    if (atomic_inc_return(&fp_bridge_disable_count) == 1) {
	fp_bridge_set(0);
    }
}
static inline void fp_bridge_enable(void) {
    if (atomic_dec_return(&fp_bridge_disable_count) == 0) {
	fp_bridge_set(1);
    }
}
...
#else
#define MAX_NETIF_RECEIVE_DEPTH 2
#endif
