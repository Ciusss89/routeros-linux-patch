...
static struct sk_buff *ppp_prepare_frame(struct ppp *ppp, struct sk_buff *skb);
...
pad_compress_skb(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (xcomp->inplace &&
		...
		if (!xcomp->lockless) ppp_xmit_lock(ppp);
		len = xcomp->compress(xc_state, orgdata,
				      skb->data, orglen, skb->len);
		if (!xcomp->lockless) ppp_xmit_unlock(ppp);
...
ppp_prepare_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if ((ppp->xstate & SC_COMP_RUN) &&
		...
		if (rcu_dereference(ppp->xc_state))
...
static int ppp_mp_explode(struct ppp *ppp, struct sk_buff *skb)
{
	...
	if (nfree == 0 || nfree < navail / 2)
		return 0; /* can't take now, leave it in queue */
