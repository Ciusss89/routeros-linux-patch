...
static int register_partitions(void)
{
	...
	unsigned nandsize = rmtd.size;
	unsigned eraseshift = ffs(rmtd.erasesize) - 1;
	unsigned partsize;
	unsigned i;
#ifdef MIPSEL
	struct nand_chip *chip = rmtd.priv;
	...
	for (i = 0; i < parts; ++i) {
	    ...
	    ret = mtd_add_partition(&rmtd, name, offset, bsize);
	    if (ret != 0) return ret;
	    offset += bsize;

	    snprintf(name, sizeof(name), "RouterBoard NAND %u Main", i + 1);
	    ret = mtd_add_partition(&rmtd, name,
	...
	if (booter_size) {
	    return mtd_add_partition(&rmtd, "RouterBoot NAND Booter",
				     0, booter_size);
	}
...
int rb_nand_probe(struct nand_chip *nand, int booter)
{
	...
	booter_size = booter ? BOOTER_PART_SIZE : 0;
