...
static void do_spi_init(struct rb400_spi *rbspi, struct spi_device *spi) {
	...
	else if (spi->chip_select == 11) {
		change_ccs112_ssr(BIT(0), BIT(1) | BIT(2));
	}
...
static unsigned inline do_swd_bit(int bit) {
	unsigned pattern = SBIT_CS_0 | SBIT_CS_2 | (bit & 1);
	SPI_REG(SPI_IO_CTRL) = pattern | SBIT_CS_1;
	CLK_HALF_DELAY();
	ndelay(spi_hclk_delay);
	SPI_REG(SPI_IO_CTRL) = pattern | SBIT_CLK;
	CLK_HALF_DELAY();
	ndelay(spi_hclk_delay);
	return SPI_REG(SPI_RDATA) & 1;
}

static void inline do_swd_init (unsigned arg) {
	unsigned i;
	for (i = 0; i < 56; ++i) {
		do_swd_bit(1);
	}
...
static unsigned inline do_swd_read(unsigned char cmd, unsigned *data) {
	unsigned i, ack = 0;
	for (i = 0; i < 8; ++i) {
		do_swd_bit(cmd >> i);
	}
	...
	ack |= do_swd_bit(1) << 1;
	ack |= do_swd_bit(1) << 2;
	if (ack == RB400_SPI_SWD_ACK_OK) {
		unsigned parity = 0, d = 0;
		for (i = 0; i < 32; ++i) {
			if (do_swd_bit(1) == 1) {
				d |= (1 << i);
				++parity;
		...
		*data = d;
		if (do_swd_bit(1) != (parity & 1)) {
			ack |= RB400_SPI_SWD_PARITY_ERROR;
		}
	}
	do_swd_bit(1);
	for (i = 0; i < 8; ++i) {
...
static unsigned inline do_swd_write(unsigned char cmd, unsigned data) {
	unsigned i, ack = 0;
	for (i = 0; i < 8; ++i) {
		do_swd_bit(cmd >> i);
	}
...
int rb400_spi_swd(struct spi_device *spi, unsigned cmd, unsigned *data)
{
	int rc;
	unsigned long flags;
	struct rb400_spi *rbspi = spi_master_get_devdata(spi->master);
	spin_lock_irqsave(&rbspi->lock, flags);
	if (rbspi->busy == 0 && rbspi->cs_wait == -1) {
	        unsigned prefix = cmd >> 16;
		set_speed(spi->max_speed_hz);
		SPI_REG(SPI_FUNC) = SBIT_FUNC_GPIO;
		SPI_REG(SPI_CTRL) = SPI_CTRL_FASTEST;
		spi_clk_low |= SBIT_CS_ALL;
		...
		if ((cmd & RB400_SPI_SWD_READ) == RB400_SPI_SWD_READ) {
			rc = do_swd_read(cmd, data);
		}
		else {
			rc = do_swd_write(cmd, *data);
		}
		SPI_REG(SPI_IO_CTRL) = SBIT_CS_ALL | SBIT_CLK;
		change_ccs112_ssr(0, BIT(0));
	}
	else {
	    rc = -EBUSY;
	}
