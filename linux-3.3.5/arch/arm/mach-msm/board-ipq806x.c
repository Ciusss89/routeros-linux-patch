...
static struct mtd_partition msm_nand_partitions[] = {
    [0] = {
...
struct flash_platform_data msm_nand_data = {
	.version = VERSION_2,
	.parts = msm_nand_partitions,
	.nr_parts = ARRAY_SIZE(msm_nand_partitions),
};
...
static const struct gpio_led rb3011_leds[] __initconst = {
	{
		.name = "user-led",
		.gpio = USER_LED_GPIO,
	},
...
static const struct gpio_led_platform_data rb3011_led_data __initconst = {
	.leds = rb3011_leds,
	.num_leds = ARRAY_SIZE(rb3011_leds),
};

static const char *gpio_names[GPIOMUX_NGPIOS] = {
    [BUTTON_GPIO] = "button",
};

static void export_button_gpio_to_sysfs(void) {
    gpio_to_chip(BUTTON_GPIO)->names = gpio_names;
...
static void __init ipq806x_init(void)
{
	...
	if (arm_machtype == MACH_MT_RB3011) {
	    gpio_led_register_device(0, &rb3011_led_data);
	}

	export_button_gpio_to_sysfs();
...
static enum hrtimer_restart beep_callback(struct hrtimer *t) {
    ...
    gpio_set_value(BEEP_GPIO, (state = !state));
...
static int __init get_board_type(char *s) {
    char name[64];
    sscanf(s, "=%s ", name);
...
late_initcall(disable_watchdog);
